...
export interface KeytermState {
  ...
  view: KeytermView;
  search: string;
  lastPruned: string[];    // ids deselected in the most recent auto-prune
  showPayload: boolean;
}
  ...
  switch (action.type) {
    ...
    case "ADD": {
      ...
      const term: ManagedKeyterm = {
        ...
        selected:    true,
        pinned:      false,
        priority:    0,       // will be set by rerank
      ...
      const mapped = state.terms.map((t) =>
        t.id === id
          ? {
              ...t,
              pinned: !t.pinned,
              // Pinning also selects the term
              selected: !t.pinned ? true : t.selected,
            }
          : t,
      );
      const ranked = rerank(mapped);
