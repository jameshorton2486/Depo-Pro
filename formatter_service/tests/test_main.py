...
def test_malformed_retry_cannot_replace_completed_job(monkeypatch) -> None:
    completed = {
        "jobId": "job-complete",
    ...
    FakeStore.rejected = []
    FakeStore.existing_job = completed
    monkeypatch.setenv("EXPORT_ARTIFACT_BUCKET", "synthetic-exports")
    monkeypatch.setattr("formatter_service.main.CloudStorageExportStore", FakeStore)

    response = TestClient(create_app()).post(
        "/tasks/format",
        json={"jobId": "job-complete", "request": {"transcriptId": "transcript-001"}},
    )

    assert response.status_code == 200
    assert response.json() == completed
    assert FakeStore.rejected == []
