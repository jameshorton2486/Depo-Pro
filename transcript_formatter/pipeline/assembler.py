...
def _build_speaker_role_map(utterances: List[Dict]) -> Dict[int, str]:
    ...
    for utterance in utterances:
        speaker_counts[int(utterance.get("speaker", 0) or 0)] += _count_utterance_words(utterance)
    ...
    for idx, (speaker_id, _) in enumerate(ranked_speakers):
        role_map[speaker_id] = ROLE_SEQUENCE[idx] if idx < len(ROLE_SEQUENCE) else f"SPEAKER {speaker_id}"
    return role_map
