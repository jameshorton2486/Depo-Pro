...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error: userError } = await supabase.auth.getUser(authorization.slice("Bearer ".length));
    if (userError) {
      throw new AdapterError(401, "unauthorized");
    }
    ...
    try {
      ...
      throw new AdapterError(400, error instanceof Error ? error.message : "adapter request is invalid");
    }
    ...
    if (payload.action === "get") {
      return respond(200, (await requireStoredJob(payload.jobId, payload.transcriptId)).value.job);
    }

    return respond(200, await cancelExport(payload.jobId, payload.transcriptId));
  } catch (error) {
...
async function createExport(
  ...
  const transcript = await requireTranscriptAccess(supabase, request.transcriptId);
  await requirePersistedCertification(supabase, transcript.case_id);

  const jobId = await deterministicIdentifier("export", `${request.transcriptId}:${request.idempotencyKey}`);
  const existing = await readStoredJob(jobId);
  if (existing) {
    try {
      assertJobTranscript(existing.value.job, request.transcriptId);
    } catch {
      throw new AdapterError(409, "Idempotency key already belongs to another transcript");
    }
...
async function cancelExport(jobId: string, transcriptId: string): Promise<ExportJob> {
  ...
  const taskName = taskResourceName(await deterministicIdentifier("export", `${transcriptId}:${stored.value.idempotencyKey ?? ""}`));
  const response = await googleRequest(`https://cloudtasks.googleapis.com/v2/${taskName}`, { method: "DELETE" });
...
async function requireTranscriptAccess(
  ...
  const byTranscript = await supabase.from("transcripts").select(fields).eq("transcript_id", routeId).maybeSingle();
  ...
  const byJob = await supabase.from("transcripts").select(fields).eq("job_id", routeId).maybeSingle();
  if (byJob.error) {
    throw new AdapterError(500, "failed to load transcript");
  }
  if (!byJob.data) {
...
async function requirePersistedCertification(
  ...
  if (!certification?.certification_date) {
    throw new AdapterError(409, "persisted certification is required before export");
  }
}

async function dispatchFormatterTask(jobId: string, request: ExportServiceRequest): Promise<void> {
  const body = bytesToBase64(new TextEncoder().encode(JSON.stringify({ jobId, request })));
  const task = {
    task: {
  ...
  const response = await googleRequest(
    `https://cloudtasks.googleapis.com/v2/projects/${encodeURIComponent(config.gcpProject)}/locations/${encodeURIComponent(config.gcpLocation)}/queues/${encodeURIComponent(config.queue)}/tasks`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function requireStoredJob(jobId: string, transcriptId: string): Promise<StoredJobVersion> {
...
async function writeStoredJob(jobId: string, value: StoredJob, generation: string): Promise<string> {
  const objectName = jobObjectName(jobId);
  const url = `https://storage.googleapis.com/upload/storage/v1/b/${encodeURIComponent(config.bucket)}/o?uploadType=media&name=${encodeURIComponent(objectName)}&ifGenerationMatch=${encodeURIComponent(generation)}`;
...
async function deleteStoredJob(jobId: string, generation: string): Promise<void> {
  const response = await googleRequest(`${storageObjectUrl(jobId, false)}&ifGenerationMatch=${encodeURIComponent(generation)}`, { method: "DELETE" });
...
function storageObjectUrl(jobId: string, media: boolean): string {
  return `https://storage.googleapis.com/storage/v1/b/${encodeURIComponent(config.bucket)}/o/${encodeURIComponent(jobObjectName(jobId))}?alt=${media ? "media" : "json"}`;
}
...
async function deterministicIdentifier(prefix: string, value: string): Promise<string> {
  const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(value));
  return `${prefix}-${Array.from(new Uint8Array(digest)).map((byte) => byte.toString(16).padStart(2, "0")).join("").slice(0, 32)}`;
}
