    ...
    const outcome = await advanceOrFinalizeMultifileJob({
      ...
      finalize: async () => {
        ...
        if (!canonicalIntegrity.integrity_passed) {
          await persistCanonicalManualReviewTranscript(serviceClient, job, merged.segments, merged.normalized, deepgramRequestId, finalArtifact.path, finalArtifact.checksum, canonicalIntegrity);
          await updateJob(serviceClient, job.id, {
            status: "failed",
            response_path: finalArtifact.path,
            error: buildCanonicalIntegrityFailureMessage(canonicalIntegrity),
          });
          return { status: "needs_manual_review" as const, responsePath: finalArtifact.path };
        }
