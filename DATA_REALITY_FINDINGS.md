...
| M | PARTIAL | `TRANSCRIPT_FIDELITY_BACKLOG.md` is absent; fallback coverage can be derived from `ERROR_COVERAGE_MATRIX.md` and the canonical changelogs. |
...
`TRANSCRIPT_FIDELITY_BACKLOG.md` is absent from the working tree. I used `ERROR_COVERAGE_MATRIX.md` and the canonical changelogs as the fallback catalog.
...
The unresolved gap still called out in `ERROR_COVERAGE_MATRIX.md` is the lack of a paragraph-boundary entity for intra-testimony hard breaks.
