...
- focused Export Adapter/UI protocol suite: 18 targeted tests passed for the latest staging and UI fixes;
- full repository suite: 735 tests passed with `STAGE_S_WRITE=1`;
...
- deployed versions: `export-adapter` v9 and `export-adapter-relay` v1;
...
Production end-to-end acceptance passed after the relay deployment on July 22, 2026. Fabricated case `RC17C-RELAY-CASE-84fcaa8fd783`, transcript `rc17c-relay-transcript-84fcaa8fd783`, and job `export-02378a0cb4fc62dd577e67027a67feb6` traversed Application credentials -> Export Adapter -> staged GCS request object -> Cloud Tasks relay -> private Formatter Service -> Cloud Storage. The inline formatter body would have been 1,252,365 bytes; the relay task body was 205 bytes. Observed job lifecycle was `QUEUED -> PROCESSING -> COMPLETED`, with DOCX/PDF signed artifacts and duplicate idempotent create returning the same completed job. The staged request object `exports/requests/export-02378a0cb4fc62dd577e67027a67feb6.json` was deleted after terminal formatter response; the completed job object remains at `exports/jobs/export-02378a0cb4fc62dd577e67027a67feb6.json` generation `1784729045708531`.
