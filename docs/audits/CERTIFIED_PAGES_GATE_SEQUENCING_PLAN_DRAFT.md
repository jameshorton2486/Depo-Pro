...
## R. GATE 2 STAGE A — EXECUTED 2026-08-11 (reversible read/projection canary; PONR NOT crossed)
Human-authorized (Stage A only). Read/projection canary via the NARROWEST reversible mechanism.
- **Canary mechanism:** the per-call `persistedLineTypeEnabled` override (request/test-only) run in a LOCAL read-only harness — NOT a global flag flip, NOT a deploy, NOT a write. The global `PERSISTED_LINE_TYPE_ENABLED` stayed `false`. No new parallel authority created — the override already exists in code (built for the Workspace==export parity harness).
- **Global flag necessary?** NO — the per-call override sufficed (narrowest viable mechanism).
- **Canary dataset:** one real production transcript `tr_1786372056908_hyjqv3` (1,757 utterances, 13,952 words, 7 speakers), fetched read-only to scratchpad; content never surfaced or committed, purged after. Pre-canary ground truth: 44 applied, 13,169 utterances all UNREVIEWED, 0 OVERRIDDEN, 0 line_type populated, 0 corrections; backup fresh.
- **Chain exercised:** existing persisted state -> Working Transcript projection -> FinalizedTranscriptModel -> CFE -> PaginationMap with `persistedLineTypeEnabled=true`. Coherent: 1,757 utterances -> 1,757 paginated lines across 259 pages; all utterances covered; every coord 1<=line<=linesPerPage.
- **Old-vs-new comparison:** the enabled model DEEP-EQUALS the legacy (disabled) model — identical word coverage, ordering, speaker identity, Q/A structure, paragraph counts/kinds, page/line coordinates, Working Transcript text (`identical_enabled_vs_disabled: true`). No divergence: production has no persisted reviewed structure (0 corrections, null line_type), so `deriveWorkingTranscript(enabled=true)` returns the document unchanged. The projection is a safe no-op on current data; divergence begins only when reviewers create new structure (Stage B/PONR).
- **Zero-write proof:** production fingerprints identical post-canary (44 applied; 13,169 utterances all UNREVIEWED; 0 OVERRIDDEN; 0 line_type; 0 manually_reassigned; 0 corrections/decisions; canary transcript 13,952 words / 1,757 utterances). Only local computation over a read-only snapshot; no INSERT/UPDATE/DELETE.
- **Rollback/reversibility (R1):** nothing to revert — global flag never flipped, nothing deployed, nothing persisted; the canary ran alongside the legacy path production still uses. Disabling = stop running the harness. No database restoration involved.
- **Result:** STAGE A PASSED. Persisted-line_type projection operates correctly against real production data, byte-identical to the legacy authority on current data, zero writes, fully reversible.
- **Residual authorities (unchanged):** legacy `qaFixer`/`keepRawLabels` remain the ACTIVE structural authority (flag off); persisted line_type is validated-but-inactive.
